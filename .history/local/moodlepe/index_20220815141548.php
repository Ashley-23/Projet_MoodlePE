...
if (isset($roleid)) {
    ...
    if ($roleid == $verify_role_id) {
        ...
        // ****************************************************************************************************************************************************************************************************AND u.id!='$userid'
        $userlist = $mysql->query("SELECT u.id, u.firstname ,u.lastname ,u.username, u.email,c.name  FROM mdl_cohort c JOIN mdl_cohort_members m  ON c.id = m.cohortid JOIN mdl_user u  ON u.id = m.userid WHERE m.cohortid='$cohortid' ", MYSQLI_USE_RESULT);
                    ...
                    <tbody>
                        ...
                        ?>
                            <td class="cell c5" style="" id="yui_3_17_2_1_1660560784812_579">
                                <a href="" onclick="getid('<?php echo $user['username'] ?>');"><i class=' icon fa fa-trash fa-fw' title='Supprimer' role='img' aria-label='Supprimer'></i></a>
                                <a href="" onclick="check('<?php echo $user['username'] ?>');"><i class='icon fa fa-cog fa-fw' title='Modifier' role='img' aria-label='Modifier'></i></a>
                            </td>
                        <?php
                            echo "</tr>";
                        }
