...
if (isset($_GET['mod'])) {
    ...
    if ($modification == "ok") {
        echo "<script>
            alert('La modification a été effectué avec succès')
        </script>";
    }
}
...
if (isset($_GET['create'])) {
    ...
    if ($modification == "ok") {
        echo "<script>
            alert('La création a été effectué avec succès')
        </script>";
    }
}
...
if (isset($_GET['moderreur'])) {
    ...
    if ($moderreur == "ok") {
        echo "<script>
            alert('Il y a eu une erreur dans la modification')
        </script>";
    }
}
...
?>
        ...
        <!-- STYLE CSS -->
        <script src="/local/moodlepe/sweetalert/src/sweetalert2.js" type='text/javascript'></script>
        <script src="/local/moodlepe/sweetalert/src/SweetAlert.js" type='text/javascript'></script>
        <link rel="stylesheet" href="/local/moodlepe/sweetalert/src/sweetalert2.scss">
                    ...
                    <tbody>
                        ...
                        ?>
                            <td class="cell c5" style="" id="yui_3_17_2_1_1660560784812_579">
                                ...
                                <!--  -->
                                <!-- <a class="icon fa fa-eye fa-fw " onclick="resetpassword('<?php echo $user['username'] ?>');" title="Réinitialiser le mot de passe" role="img" aria-label="Réinitialiser le mot de passe"></a> -->

                                <a class="icon fa fa-eye fa-fw " title="Réinitialiser le mot de passe" role="img" aria-label="Réinitialiser le mot de passe" data-toggle="modal" data-target="#exampleModal"></a>

                                <!-- <a class="icon fa fa-cog fa-fw " title="Modifier" role="img" aria-label="Modifier" onclick="getid('<?php echo $user['username'] ?>');"></a> -->


                                <a class=" icon fa fa-trash fa-fw " title="supprimer" role="img" aria-label="Supprimer l'utilisateur" data-toggle="modal" data-target="#supprModal"></a>
...
echo " <div class='modal-dialog' role='document'>";
     echo "<div class='modal-content'>";
        echo " <div class='modal-header'>";
             echo "<h5 class='modal-title' id='exampleModalLabel'>Réinitialiser le mot de passe</h5>
             <button type='button' class='close' data-dismiss='modal' aria-label='Close'>
                 <span aria-hidden='true'>&times;</span>
             </button>
         </div>";
         echo "<div class='modal-body'>
             Voulez-vous vraiment réinitialiser le mot de passe de l'utilisateur  $user['username'] ?
         </div>";
         echo "<div class='modal-footer'>
             <button type='button' class='btn btn-secondary' data-dismiss='modal'>Annuler</button>";
            echo "<button type='button' class='btn btn-primary' onclick='window.location.href = '/moodlepe/local/moodlepe/user/resetpassword.php?username=<?php echo $user['username'] ?>';'>Continuer</button>";
         echo "</div>
     </div>
 </div>
</div>";
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        Voulez-vous vraiment réinitialiser le mot de passe de l'utilisateur <?php echo $user['username'] ?> ?
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Annuler</button>
                        <button type="button" class="btn btn-primary" onclick="window.location.href = '/moodlepe/local/moodlepe/user/resetpassword.php?username=<?php echo $user['username'] ?>';">Continuer</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal fade" id="supprModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        Voulez-vous vraiment supprimer l'utilisateur <?php echo $user['username'] ?> ?
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Annuler</button>
                        <button type="button" class="btn btn-primary" onclick="window.location.href ='/moodlepe/local/moodlepe/user/delete.php?username=<?php echo $user['username'] ?>';">Continuer</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal fade" id="myreset" tabindex="-1" role="dialog" aria-labelledby="myresetlabel">
            <div class=" modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="myresetlabelLabel">Réinitialiser le mot de passe</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
