...
if (isset($_POST['username']) || isset($_POST['lastname']) || isset($_POST['firstname']) || isset($_POST['email']) || isset($_POST['entreprise']) || isset($_POST['password']) || isset($_POST['entrepriseid'])) {
    ...
    if (($username_result == 1 && $firstname_result == 1) || $lastname_result == 1 || $email_result == 1) {
        ...
        if ($check_cohort_member != 'NULL') {
            ...
            $mdl_local_moodlepe_suppr  = $mysql->query("INSERT INTO mdl_local_moodlepe_suppr SET username, firstname, lastname,email ,password ,enterprise ,timecreated ,active ,suppr ,suppr_user_id ,userid)VALUES ('','$username', '$firstname', '$lastname','$email', '$password', '$entreprise','$timecreated', 't','f', '$authuserid', '$user_id')", MYSQLI_USE_RESULT);
