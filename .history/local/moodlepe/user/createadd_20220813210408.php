...
if (isset($_POST['username']) || isset($_POST['lastname']) || isset($_POST['firstname']) || isset($_POST['email']) || isset($_POST['entreprise']) || isset($_POST['password'])) {
    ...
    $mdl_user  = $mysql->query("INSERT INTO mdl_user ( auth, confirmed, deleted, timezone,  username, firstname, lastname,mnethostid, lang,  password, email, timecreated, timemodified )
     VALUES ('$auth', '$confirmed', '$deleted','$timezone','$username', '$firstname', '$lastname','$mnethostid', '$lang', '$password', '$email',  '$timecreated', '$timemodified')", MYSQLI_USE_RESULT);)", MYSQLI_USE_RESULT);
