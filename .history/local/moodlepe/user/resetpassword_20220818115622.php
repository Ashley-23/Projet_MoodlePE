...
if (isset($_GET['username'])) {
    ...
    $p = str_replace(' ', '', $lower_lastname);
    $resetpassword;

    echo "</br> resetpassword : " . $resetpassword;
    echo "</br> p : " . $resetpassword;
    $final_password = hash_internal_user_password($resetpassword);
    echo "</br> finalpassword : " . $final_password;
    ...
    // //REINITIALISE LE MOT DE PASSE DE L'UTILISATEUR DANS LA TABLE DE NOTRE PLUGIN LOCAL
    // $update_mdl_local_moodlepe_suppr = "UPDATE mdl_local_moodlepe_suppr SET password='$final_password ' WHERE userid='$userid'";
    // $mysql->query($update_mdl_local_moodlepe_suppr);



    // header("Location: /moodlepe/local/moodlepe/index.php?reset=ok");
    die();
} else {
