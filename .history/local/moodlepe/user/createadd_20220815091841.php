...
if (isset($_POST['username']) || isset($_POST['lastname']) || isset($_POST['firstname']) || isset($_POST['email']) || isset($_POST['entreprise']) || isset($_POST['password']) || isset($_POST['entrepriseid'])) {
    ...
    $check_user_username = $mysql->query("SELECT * FROM mdl_user WHERE UPPER(username='$username'");
    $check_user_firstname = $mysql->query("SELECT * FROM mdl_user WHERE firstname='$firstname'");
    $check_user_lastname = $mysql->query("SELECT * FROM mdl_user WHERE lastname='$lastname'");
    $check_user_email = $mysql->query("SELECT * FROM mdl_user WHERE email='$email'");
    ...
    if ($username_result == 1 && $firstname_result == 1 && $lastname_result == 1 && $email_result == 1) {
        ...
        echo "</br> Nom saisie : " . $lastname;
        echo "</br> Prénom saisie : " . $firstname;
        echo "</br> Nom d'utilisateur saisie : " . $username;
        echo "</br> Email saisie : " . $email;
    } else {
        echo "</br> Je n'existe pas encore";
    }
