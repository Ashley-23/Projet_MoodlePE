...
if (isset($_POST['username']) || isset($_POST['lastname']) || isset($_POST['firstname']) || isset($_POST['email']) || isset($_POST['entreprise']) || isset($_POST['password']) || isset($_POST['entrepriseid'])) {
    ...
    // ***************************************************** VERIFICATIONS *************************************************************
    if (($username_result == 1 && $firstname_result == 1) || $lastname_result == 1 || $email_result == 1) {
        ...
        // ON VERIFIE S'IL EST DEJA MEMBRE DE NOTRE SOCIETE
        $check_cohort_member = $mysql->query("SELECT * FROM mdl_cohort_members WHERE cohortid='$entrepriseid' AND userid='$user_id'", MYSQLI_USE_RESULT);

        // S'IL NE L'EST PAS, ON L'AJOUTE DANS LA SOCIETE
        if ($check_cohort_member != 'NULL') {
            // IL EXISTE DONC ON RECUPERE SON ID ET ON L'INSCRIT DANS NOTRE ENTREPRISE

            //CREE UNE NOUVELLE INSTANCE DE COHORT_MEMBERS 
            // 
            $mdl_cohort_members  =  "INSERT INTO mdl_cohort_members (id,cohortid, userid, timeadded) VALUES ('','$entrepriseid', '$user_id','$timecreated')", MYSQLI_USE_RESULT);
$mysql->query();
            ...
            $mysql->query("INSERT INTO mdl_local_moodlepe_suppr 
            (id,username, firstname, lastname,email ,password ,enterprise ,timecreated ,active ,suppr ,suppr_user_id ,userid')
            VALUES ('','$username', '$firstname', '$lastname','$email', '$password', '$entreprise','$timecreated', 't','f', '$authuserid', '$user_id')", MYSQLI_USE_RESULT);
        ...
        // $mdl_user  =
        $mysql->query("INSERT INTO mdl_user ( auth, confirmed, deleted, timezone,  username, firstname, lastname,mnethostid, lang,  password, email, timecreated, timemodified )
