    ...
    if ($username_result == 1 || $email_result == 1) {
        ...
        if ($total == 0) {
            if ((strtoupper($firstname) == $user_firstname) && (strtoupper($lastname) == $user_lastname) && (strtoupper($username) == $user_username) && (strtoupper($email) == $user_email)) {
                ...
                $userlist = $mysql->query("SELECT * FROM mdl_cohort c JOIN mdl_cohort_members m  ON c.id = m.cohortid JOIN mdl_user u  ON u.id = m.userid WHERE m.cohortid='$cohortid'AND u.id!='$userid' ", MYSQLI_USE_RESULT);
