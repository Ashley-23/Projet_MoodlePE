...
if (isset($_POST['username']) || isset($_POST['lastname']) || isset($_POST['firstname']) || isset($_POST['email']) || isset($_POST['entreprise']) || isset($_POST['password']) || isset($_POST['entrepriseid'])) {
    ...
    // ***************************************************** VERIFICATIONS *************************************************************
    if ($username_result == 1 || $email_result == 1) {
        ...
        if ($total == 0) {
            if ((strtoupper($firstname) == $user_firstname) && (strtoupper($lastname) == $user_lastname) && (strtoupper($username) == $user_username) && (strtoupper($email) == $user_email)) {
                // if (empty($check_cohort_member)) {

                // ***************************DIFFERENT DES ADMINISTRATEURS D'ENTREPRISES
                $userlistcount = $mysql->query("SELECT COUNT(*) as total FROM mdl_local_moodlepe_user", MYSQLI_USE_RESULT);
                while ($row = mysqli_fetch_assoc($userlistcount)) {
                    $cpt = $row['total'];
                }

                if ($cpt != 0) {
                    echo "</br> Je suis différent de 0  </br>";
                    $userlist = $mysql->query("SELECT COUNT(*) as total FROM mdl_local_moodlepe_user WHERE UPPER(email) = UPPER('$email')", MYSQLI_USE_RESULT);

                    while ($row1 = mysqli_fetch_assoc($userlist)) {
                        $us = $row1['total'];
                    }

                    if ($us != 0) {

                        header("Location: /moodlepe/local/moodlepe/index.php?emailoruserexist=ok");
                    }
                }
            ...
            header("Location: /moodlepe/local/moodlepe/index.php?emailoruser=ok");
        }
        // 
