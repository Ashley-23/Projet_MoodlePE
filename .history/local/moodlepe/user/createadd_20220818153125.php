    ...
    if ($username_result == 1 || $email_result == 1) {
        ...
        if ($total == 0) {
            if ((strtoupper($firstname) == $user_firstname) && (strtoupper($lastname) == $user_lastname) && (strtoupper($username) == $user_username) && (strtoupper($email) == $user_email)) {
                ...
                $userlistcount = $mysql->query("SELECT COUNT(*) as total FROM mdl_local_moodlepe_user WHERE UPPER(email)= UPPER($email)", MYSQLI_USE_RESULT);
                while ($row = mysqli_fetch_assoc($userlistcount)) {
                    $total = $row['total'];
                }
                $userlist = $mysql->query("SELECT * FROM mdl_local_moodlepe_user WHERE UPPER(email)= UPPER($email)", MYSQLI_USE_RESULT);


                $users = array();
                while ($row = mysqli_fetch_assoc($userlist)) {
                    $users[] = array('id' => $row['id'], 'nom' => $row['lastname'], 'prenom' => $row['firstname'], 'username' => $row['username'], 'email' => $row['email']);
                }
